...
if(hasInterface)then{'' spawn{
	...
	VoN_Event_fnc = {
		...
		if((VoN_channelId select 1) >= 0 && !((VoN_channelId select 1) in VC_allowedChannels))then{
			playSound "Alarm";	//Replace "Alarm" with "Hint3" for a more subtle audio hint
			cutText[format["Sprechen im %1 ist nicht erlaubt!\n Channel auf Directchat umgestellt", str localize(VoN_channelId select 0), actionKeysNames ["nextChannel",1], actionKeysNames ["prevChannel",1]], "PLAIN DOWN", 1];
			setCurrentChannel 5;
		};
	};
