...
[] spawn {
	...
	while {true} do {
		...
		//
		// 15 minutes is the maximum, in a car or not
		//
...
while {player GVAR  "restrained"} do {
	...
	// player must not enter gunner seat when restrained
	// thx to fabiox2011 for finding this!
	if((vehicle player != player) && life_disable_getOut) then {
		if (_vehicle turretUnit [0] == player) then {
			player action["eject",vehicle player];
			player moveInCargo _vehicle;
		};
		
		if (_vehicle turretUnit [1] == player) then {
			player action["eject",vehicle player];
			player moveInCargo _vehicle;
		};
		
		if (_vehicle turretUnit [2] == player) then {
			player action["eject",vehicle player];
			player moveInCargo _vehicle;
		};
		
		if (_vehicle turretUnit [3] == player) then {
			player action["eject",vehicle player];
			player moveInCargo _vehicle;
		};
	};
